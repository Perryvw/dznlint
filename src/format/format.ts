                ...
                do {
                    switch (c2.nodeType) {
                        ...
                        case "blocking":
                            ...
                            break;
                        case "invariant":
                            formatInvariant(c2.pos(), formatter);
                            break;
                        // generics
                        case "comment":
                            formatComment(c2.currentNode, formatter);
    ...
    do {
        switch (cursor.nodeType) {
            ...
            case "action_or_call_statement":
                formatActionOrCallStatement(cursor.pos(), formatter);
                break;
            case "assign":
...
function formatInvariant(cursor: Grammar.CursorPosition<Grammar.invariant_Node>, formatter: Formatter) {
    cursor.gotoFirstChild();
    do {
        switch (cursor.nodeType) {
            case "invariant":
...
function formatActionOrCallStatement(
    cursor: Grammar.CursorPosition<Grammar.action_or_call_statement_Node>,
    formatter: Formatter
) {
    cursor.gotoFirstChild();
    formatter.requirePrecedingNewLine();
