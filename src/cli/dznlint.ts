...
if (cliArguments.arguments.format) {
    ...
    // Lint resolved files
    api.lintFiles(inputFiles, configuration, { includePaths: cliArguments.arguments.includePaths }).then(result => {
        const counts = {
        ...
        if (
            counts[DiagnosticSeverity.Error] + counts[DiagnosticSeverity.Warning] + counts[DiagnosticSeverity.Hint] ===
            0
        )
